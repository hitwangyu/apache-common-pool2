...
public class GenericObjectPool<T> extends BaseGenericObjectPool<T> implements ObjectPool<T>,
    GenericObjectPoolMXBean, UsageTracking<T> {
  ...
  public void setAbandonedConfig(AbandonedConfig abandonedConfig) throws IllegalArgumentException {
    if (abandonedConfig == null) {
      ...
      this.abandonedConfig.setRemoveAbandonedOnMaintenance(abandonedConfig
  ...
  public T borrowObject(long borrowMaxWaitMillis) throws Exception {
    assertOpen(); // 确定对象池已经打开

    AbandonedConfig ac = this.abandonedConfig;
    if (ac != null && ac.getRemoveAbandonedOnBorrow() && (getNumIdle() < 2)
        && (getNumActive() > getMaxTotal() - 3)) {
      removeAbandoned(ac); // 对于超过一定时间未被使用的对象，标记为abandoned
    }

    PooledObject<T> p = null;

    // Get local copy of current config so it is consistent for entire
    // method execution。当无可用对象时是否阻塞
    ...
    while (p == null) {
      ...
      if (blockWhenExhausted) {
        ...
        if (p == null) {
          if (borrowMaxWaitMillis < 0) { // 阻塞时间<0则一直阻塞
    ...
    if (idleObjects.size() > 0) {
      ...
      synchronized (evictionLock) {
        EvictionConfig evictionConfig =
            new EvictionConfig(getMinEvictableIdleTimeMillis(),
  ...
  private PooledObject<T> create() throws Exception {
    ...
    long newCreateCount = createCount.incrementAndGet(); // 保证正在创建的+已经创建的 不超过MaxTotal
  ...
  /*
   * All of the objects currently associated with this pool in any state. It excludes objects that
   * have been destroyed. The size of {@link #allObjects} will always be less than or equal to
   * {@link #_maxActive}. Map keys are pooled objects, values are the PooledObject wrappers used
   * internally by the pool.
   */
  private final Map<IdentityWrapper<T>, PooledObject<T>> allObjects =
      new ConcurrentHashMap<IdentityWrapper<T>, PooledObject<T>>();
  /*
   * The combined count of the currently created objects and those in the process of being created.
   * Under load, it may exceed {@link #_maxActive} if multiple threads try and create a new object
   * at the same time but {@link #create()} will ensure that there are never more than {@link
   * #_maxActive} objects created at any one time.
   */
  private final AtomicLong createCount = new AtomicLong(0);
  // 空闲对象队列。队列实现和jdk一模一样... 不知道为啥不直接引concurrent包...
