 ...
 * <ol>
 * <li>
 * {@link #makeObject} is called whenever a new instance is needed.</li>
 * <li>
 ...
 * instance.</li>
 * <li>
 * {@link #passivateObject} is invoked on every instance when it is returned to the pool.</li>
 * <li>
...
public interface PooledObjectFactory<T> {
  /**
   * Create an instance that can be served by the pool and wrap it in a {@link PooledObject} to be
   * managed by the pool.
   *
   ...
   * Ensures that the instance is safe to be returned by
   * the pool.
   *
   * @param p a {@code PooledObject} wrapping the instance to be validated
   *
   * @return <code>false</code> if <code>obj</code> is not valid and should be dropped from the
   *         pool, <code>true</code> otherwise.
   */
  boolean validateObject(PooledObject<T> p);

  /**
   * "激活"对象,当Pool中决定移除一个对象交付给调用者时,额外的"激活"操作; 
   ...
   * Reinitialize an instance to
   * be returned by the pool.
   *
   ...
   * 归还对象到对象池时注意做数据清理，防止后面获取此对象的线程拿到脏数据 
   * Uninitialize an instance to be returned
   * to the idle object pool.
   *
